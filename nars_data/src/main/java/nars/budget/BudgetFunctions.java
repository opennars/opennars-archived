...
public final class BudgetFunctions extends UtilityFunctions {
	 ...
	 */
	public static Budget revise(Truth tTruth, Truth bTruth, Truth truth,
			Budget tb) {
	...
	// *
	// *
	// * @param factor linear interpolation factor; 1.0: values are applied
	// fully, 0: values are not applied at all
	...
	// final Activating mode, final float factor) {
	// switch (mode) {
	// /*case Max:
	// receiver.max(amount);
	// break;*/
	...
	// float durability = aveAri(receiver.getDurability(),
	// amount.getDurability());
